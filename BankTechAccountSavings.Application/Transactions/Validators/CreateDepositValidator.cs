...
namespace BankTechAccountSavings.Application.Transactions.Validators
{
    public class CreateDepositValidator : AbstractValidator<CreateDeposit>
    {
        public CreateDepositValidator()
        {
            ...
            RuleFor(deposit => deposit.DestinationProductNumber)
                .NotEmpty().WithMessage("Account Number is required.").GreaterThan(0).WithMessage("The Account number must be greater than zero.");
